 ...
 * 符号なしおよび指数付き数値コンバータ。 
 * <p>
 * {@link NumberString}を継承し、符号なしおよび指数付き数値であるかどうかの形式チェックおよび変換を行う。
 * </p>
 * <p>
 ...
 * @author TIS
 */
public class ExponentialNumberString extends NumberString {

    /** 符号なし数値のパターン */
    private static final Pattern PATTERN = Pattern.compile("^([0-9][0-9]*)?[0-9](\\.[0-9]*[0-9])?([eE][-+]?[0-9]+)?$");

    /**{@inheritDoc}
     * <p/>
     * {@link ExponentialNumberString}では、入出力データが符号なしおよび指数付き数値であることのチェックを行う。
     */
    @Override
